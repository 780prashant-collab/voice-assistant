...
import numpy as np
import psutil
from google_gemini import set_api_key, generate, play
from google_gemini import generate, play
from google_gemini import set_api_key
...
def engine_talk(query):
    audio = generate(
        text=query, 
        voice='Grace',
        model="google_gemini_monolingual_v1"
    )
    play(audio)
...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak("Good morning Prashant, it's {day} and the time is {t}")
    elif(hour>=12) and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon Prashant, it's {day} and the time is {t}")
    else:
        speak(f"Good evening Prashant, it's {day} and the time is {t}")       
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if ('facebook' in query) or ('youtube' in query) or ('whatsapp' in query) or ('instagram' in query):
            social_media(query)
        elif ("university time" in query) or ("schedule" in query):
            schedule()    
